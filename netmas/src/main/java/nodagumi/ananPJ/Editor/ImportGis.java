...
*/
            ...
            /*
            MapNode[] mapNodes = new MapNode[nodes.size()];
            int i = 0;
            for (String key : nodes.keySet()) {
                mapNodes[i] = nodes.get(key);
                i++;
            }
            for (i = 0; i < nodes.size(); i++) {
                for (int j = i + 1; j < nodes.size(); j++) {
                    if (mapNodes[i].getX() == mapNodes[j].getX() &&
                            mapNodes[i].getY() == mapNodes[j].getY()) {
                        System.err.println("node " + mapNodes[i].ID + " and " +
                                mapNodes[j].ID + " are same map nodes.");
                    }
                }
            }
            */
