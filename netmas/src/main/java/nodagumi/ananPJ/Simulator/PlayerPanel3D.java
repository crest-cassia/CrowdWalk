...
public class PlayerPanel3D extends SimulationPanel3D
        implements Serializable {
    private static final long serialVersionUID = -4438167798239555983L;
    /*
    ...
    */
    public PlayerPanel3D(EvacuationModelBase _model, JFrame _parent) {
    ...
    */
    public void deserialize(EvacuationModelBase _model, JFrame _parent) {
        super.deserialize(_model, _parent);
    }
    /*
    ...
    */
    ...
    protected void setupExtraContents() {
        // extra menu items
        menu_item_agent_color_speed = new CheckboxMenuItem(
                "Change agent color depending on speed", false);
        ...
        Menu menu_pollution_color = new Menu(
                "Change pollution color settings");
    ...
    protected void setup_control_panel() {
        ...
        agent_size_control = new JScrollBar(JScrollBar.HORIZONTAL, (int)(agent_size * 10),
                1, 1, 100);
        agent_size_control.addAdjustmentListener(new AdjustmentListener() {
            public void adjustmentValueChanged(AdjustmentEvent e) {change_agent_size();}
        });
    ...
    static class CurrentCameraPosition implements Comparable<CurrentCameraPosition> {
    ...
    private void save_camera_position_list() {
        FileDialog fd = new FileDialog(parent, "Save camera position list",
                FileDialog.SAVE);
        fd.setVisible (true);
    ...
    private void load_camera_position_list() {
        FileDialog fd = new FileDialog(parent, "Open camera position list", FileDialog.LOAD);
        fd.setVisible (true);
    ...
    */
    ...
    private void group_from_agent(EvacuationAgent agent) {
        ...
        app.setColoringAttributes(new ColoringAttributes(WHITE, ColoringAttributes.FASTEST));
        app.setCapability(Appearance.ALLOW_COLORING_ATTRIBUTES_WRITE);
        TransparencyAttributes ta = new TransparencyAttributes(TransparencyAttributes.NICEST,
                agent_transparency);
    ...
    private TransformGroup groupFromPollution(PollutedArea pollution) {
        ...
        UpdatePollution tb = new UpdatePollution(pollution, pollutionTransforms, app);
        tb.setSchedulingBounds(bounds);
    ...
    */
    ...
    protected void register_map_objects() {
        ...
        if (show_gas != gas_display.NONE) {
            for (PollutedArea pollution : getPollutions()) {
                TransformGroup pollutionTransforms =
                    groupFromPollution(pollution);
        ...
        for (String key : lm.keySet()) {
            ...
            tb.setSchedulingBounds(getBoundingSphere());
    ...
    class UpdateAgent extends Behavior{
        ...
        public void processStimulus(java.util.Enumeration criteria) {
            ...
            case 0://GREEN
            {
                if (menu_item_agent_color_speed.getState()) {
                    if (!((RunningAroundPerson)agent).isPassedNode()){
                        app.setColoringAttributes(new ColoringAttributes(YELLOW, ColoringAttributes.FASTEST));
                    } else if (agent.getCurrentLink() != null &&
                            agent.getCurrentLink().getLaneWidth(agent.getDirection()) == 0) {
                        app.setColoringAttributes(new ColoringAttributes(LIGHTB, ColoringAttributes.NICEST));
                    } else {
                    ...
                    app.setColoringAttributes(new ColoringAttributes(BLUE, ColoringAttributes.FASTEST));
                } else if (agent.hasTag("APINK")){
                    app.setColoringAttributes(new ColoringAttributes(APINK, ColoringAttributes.FASTEST));
                } else if (agent.hasTag("YELLOW")){
                    app.setColoringAttributes(new ColoringAttributes(YELLOW, ColoringAttributes.FASTEST));
                } else {
                    app.setColoringAttributes(new ColoringAttributes(agent_color, ColoringAttributes.FASTEST));
                }
            }
            break;
            ...
            case 2://RED    -> PINK (Any damaged person ) -> Poisonus red
                app.setColoringAttributes(new ColoringAttributes(PRED, ColoringAttributes.FASTEST));
                break;
    ...
    protected void update_camerawork(double time) {
        if (replay_recorded_camera_position.isSelected() &&
            ...
            if (last_camera == null) {
                ...
                agent_size = (next_camera.agent_size - last_camera.agent_size) * ratio + last_camera.agent_size;
                vertical_zoom = (next_camera.vertical_zoom - last_camera.vertical_zoom) * ratio + last_camera.vertical_zoom;
