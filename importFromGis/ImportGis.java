...
public class ImportGis  {
    ...
    class ExportNodeLinkAction extends SafeAction {
        ...
        public ExportNodeLinkAction(NetworkMap _networkMap) {
            super("to NetMAS");
            putValue(Action.SHORT_DESCRIPTION, "Exports GIS map to node-link data of NetMAS");
        ...
        @Override
        public void action(ActionEvent arg0) throws Throwable {
            int make_precise = JOptionPane.showConfirmDialog(map_frame, "Make precise model?", "", JOptionPane.YES_NO_CANCEL_OPTION);
            if (make_precise == JOptionPane.CANCEL_OPTION) return;
...
/*
            group.setWest(ref.getMinX());
            group.setSouth(ref.getMinY());
            group.setEast(ref.getMaxX());
            group.setNorth(ref.getMaxY());
*/
            ...
            for (int i = 0; i < layers.length; ++i) {
                ...
                while (it.hasNext()) {
                    ...
                    double length = (Double)(feature.getAttribute("LK_LENGTH"));
                    int tpcd = Integer.parseInt(((String)(feature.getAttribute("WIDTH_TPCD"))));
                    double width = width_array[tpcd];

                    if (make_precise == JOptionPane.YES_OPTION) {
                        make_nodes_precise(the_geom, feature, group,
                                nodes,
                                length, width,
                                base_x, base_y, scale_x, scale_y);
                    } else {
                        make_nodes_simple(the_geom, feature, group,
                                nodes,
                                length, width,
                                base_x, base_y, scale_x, scale_y);
                    }
    ...
    private void make_nodes_precise(Object the_geom, SimpleFeature feature,
            MapPartGroup parent_group,
            HashMap<String, MapNode> nodes,
            double length, double width,
            double base_x, double base_y, double scale_x, double scale_y) {
        ...
        for (int j = 0; j < points.length; j++) {
            ...
            if (from != null) {
                networkMap.createMapLink(parent_group, from, node, length * ratio[j - 1], width);
            }
            from = node;
        }
    }
    ...
    private void make_nodes_simple(Object the_geom, SimpleFeature feature,
        ...
        for (int j = 0; j < 2; j++) {
            ...
            if (from != null) {
                networkMap.createMapLink(parent_group, from, node, length , width);
            }
            from = node;
        }
    }
